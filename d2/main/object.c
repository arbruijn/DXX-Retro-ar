	...
	switch (obj->control_type)
	{
		case CT_WEAPON:
			...
			if (obj->ctype.laser_info.creation_time - GameTime64 < F1_0*(-18000))
				COPY_I(obj_rw->ctype.laser_info.creation_time , F1_0*(-18000));
			else
		...
		case CT_AI:
		{
			...
			if (obj->ctype.ai_info.dying_start_time == 0) // if bot not dead, anything but 0 will kill it
				COPY_I(obj_rw->ctype.ai_info.dying_start_time , 0);
			else
				COPY_I(obj_rw->ctype.ai_info.dying_start_time , obj->ctype.ai_info.dying_start_time - GameTime64);
			break;
		}
		...
		case CT_POWERUP:
			...
			if (obj->ctype.powerup_info.creation_time - GameTime64 < F1_0*(-18000))
				COPY_I(obj_rw->ctype.powerup_info.creation_time , F1_0*(-18000));
			else
				COPY_I(obj_rw->ctype.powerup_info.creation_time , obj->ctype.powerup_info.creation_time - GameTime64);
